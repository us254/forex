...
PRIVATE_KEY_XRAY=$(echo "$KEY_OUTPUT" | awk -F 'Private key: ' '{print $2}' | awk '{print $1}' | xargs)
PUBLIC_KEY_XRAY=$(echo "$KEY_OUTPUT" | awk -F 'Public key: ' '{print $2}' | awk '{print $1}' | xargs)
# Generate short ID
...
RESERVED_VALUES=$(echo "$WIREGUARD_OUTPUT" | awk -F 'reserved: ' '{print $2}' | awk -F 'v4: ' '{print $1}' | xargs)
